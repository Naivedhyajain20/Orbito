...
struct NotchHomeView: View {
    ...
    @Default(.frontWidgetMusic) var showMusic
    @Default(.frontWidgetSystem) var showSystem
    @Default(.frontWidgetCalendar) var showCalendar
    @Default(.frontWidgetTimer) var showTimer
    @Default(.frontWidgetClipboard) var showClipboard
    @Default(.frontWidgetQuickNote) var showQuickNote
    ...
    private var hasExtraWidgets: Bool {
        showSystem || showTimer || showClipboard || showQuickNote
    }

    private var hasMedia: Bool {
        showMusic || (showCalendar && Defaults[.showCalendar])
    }

    private var mediaPage: some View {
        HStack(alignment: .top, spacing: (shouldShowCamera && showCalendar && Defaults[.showCalendar]) ? 10 : 14) {
            if showMusic {
                MusicPlayerView(albumArtNamespace: albumArtNamespace)
                    .frame(maxWidth: (showCalendar && Defaults[.showCalendar]) ? 380 : .infinity)
            }

            if showCalendar && Defaults[.showCalendar] {
                CalendarView()
    ...
    private var paginationBar: some View {
        HStack(spacing: 6) {
            Button(action: {
                ...
                HStack(spacing: 4) {
                    Capsule()
                        .fill(selectedPage == 0 ? Color.white : Color.white.opacity(0.3))
                        .frame(width: selectedPage == 0 ? 14 : 5, height: 4)
                    if selectedPage == 0 {
                        Text("Media")
                ...
                .padding(.horizontal, selectedPage == 0 ? 6 : 3)
                .padding(.vertical, 2)
                .background(
                    Capsule()
                        .fill(selectedPage == 0 ? Color.white.opacity(0.14) : Color.clear)
                )
            }
            .buttonStyle(PlainButtonStyle())
            ...
            Button(action: {
                ...
                HStack(spacing: 4) {
                    Capsule()
                        .fill(selectedPage == 1 ? Color.white : Color.white.opacity(0.3))
                        .frame(width: selectedPage == 1 ? 14 : 5, height: 4)
                    if selectedPage == 1 {
                        Text("Widgets")
                            .font(.system(size: 9, weight: .semibold))
                            .foregroundStyle(.white)
                ...
                .padding(.horizontal, selectedPage == 1 ? 6 : 3)
                .padding(.vertical, 2)
                .background(
                    Capsule()
                        .fill(selectedPage == 1 ? Color.white.opacity(0.14) : Color.clear)
                )
            }
            .buttonStyle(PlainButtonStyle())
        }
