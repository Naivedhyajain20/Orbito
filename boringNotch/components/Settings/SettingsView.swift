    ...
    var body: some View {
        NavigationSplitView {
            List(selection: $selectedTab) {
                ...
                NavigationLink(value: "Front Screen") {
                    Label("Front Screen", systemImage: "macwindow")
                }
...
//                }
                ...
                NavigationLink(value: "Productivity") {
                    Label("Productivity", systemImage: "list.clipboard")
                }
...
struct Appearance: View {
    ...
    var body: some View {
        Form {
            Section {
                Picker("Active Mode", selection: $modeManager.currentMode) {
            ...
            Section {
                Picker("Theme", selection: $notchTheme) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    case .minimal:
                        Text("⚡ Minimalist clean dark slate with high contrast.")
                    }
                }
                .font(.caption)
                .foregroundColor(.secondary)
...
struct Shortcuts: View {
    var body: some View {
        Form {
            ...
            Section {
                KeyboardShortcuts.Recorder("Open Notes:", name: .toggleNotesPanel)
                KeyboardShortcuts.Recorder("Open Timers:", name: .openTimers)
                KeyboardShortcuts.Recorder("Open Calculator:", name: .openCalculator)
                KeyboardShortcuts.Recorder("Open Clipboard:", name: .openClipboard)
                KeyboardShortcuts.Recorder("Open System Monitor:", name: .openSystemInfo)
                KeyboardShortcuts.Recorder("Open AI Actions:", name: .openAIActions)
            } header: {
                Text("Quick Access — Productivity Tabs")
            } footer: {
                Text("Disabled shortcuts (grayed out) mean the corresponding feature tab is turned off in Productivity settings.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }

            Section {
                KeyboardShortcuts.Recorder("Toggle Focus Mode:", name: .toggleFocusMode)
                KeyboardShortcuts.Recorder("Cycle Notch Mode:", name: .cycleNotchMode)
            } header: {
                Text("Focus & Mode")
            } footer: {
                Text("Cycle Notch Mode rotates through Normal → Minimal → Productivity → Music → Work.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
        }
