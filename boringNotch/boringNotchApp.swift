...
struct DynamicNotchApp: App {
    ...
    var body: some Scene {
        MenuBarExtra("Orbito", systemImage: "sparkle", isInserted: $showMenuBarIcon) {
            Button("Settings") {
            ...
            Menu("Notch Mode") {
                ForEach(NotchMode.allCases) { mode in
                    Button {
                        NotchModeManager.shared.currentMode = mode
                    } label: {
                        HStack {
                            if Defaults[.notchMode] == mode {
            ...
            Menu("Theme") {
                ForEach(NotchTheme.allCases) { theme in
                    Button {
                        Defaults[.notchTheme] = theme
                    } label: {
                        HStack {
                            if Defaults[.notchTheme] == theme {
                                Image(systemName: "checkmark")
                            }
                            Text(theme.rawValue)
                        }
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        KeyboardShortcuts.onKeyDown(for: .toggleNotesPanel) { [weak self] in
            guard let self = self, Defaults[.showNotes] else { return }
            Task { @MainActor in
        ...
        KeyboardShortcuts.onKeyDown(for: .openTimers) { [weak self] in
            guard let self = self, Defaults[.showTimers] else { return }
            Task { @MainActor in
                self.coordinator.currentView = .timers
                self.vm.open()
            }
        }

        KeyboardShortcuts.onKeyDown(for: .openCalculator) { [weak self] in
            guard let self = self, Defaults[.showCalculator] else { return }
            Task { @MainActor in
                self.coordinator.currentView = .calculator
                self.vm.open()
            }
        }
        ...
        KeyboardShortcuts.onKeyDown(for: .openSystemInfo) { [weak self] in
            guard let self = self, Defaults[.showSystemMonitor] else { return }
            Task { @MainActor in
                self.coordinator.currentView = .systemInfo
                self.vm.open()
            }
        }

        KeyboardShortcuts.onKeyDown(for: .openAIActions) { [weak self] in
            guard let self = self, Defaults[.showAIActions] else { return }
            Task { @MainActor in
                // Grab selected text when opening via shortcut
                let text = AIActionManager.shared.readSelectedText() ?? ""
                AIActionManager.shared.selectedText = text
                self.coordinator.currentView = .aiActions
                self.vm.open()
            }
        }

        KeyboardShortcuts.onKeyDown(for: .toggleFocusMode) {
            Task { @MainActor in
                FocusModeManager.shared.toggleFocus()
            }
        }

        KeyboardShortcuts.onKeyDown(for: .cycleNotchMode) {
            Task { @MainActor in
                NotchModeManager.shared.cycleMode()
            }
        }
