...
class BoringViewModel: NSObject, ObservableObject {
    ...
    init(screenUUID: String? = nil) {
        ...
        Defaults.publisher(.customOpenHeight)
            .receive(on: RunLoop.main)
            .sink { [weak self] change in
                guard let self = self, self.notchState == .open else { return }
                withAnimation(.spring(response: 0.35, dampingFraction: 0.8)) {
                    self.notchSize = CGSize(width: openNotchSize.width, height: CGFloat(change.newValue))
                }
            }
            .store(in: &cancellables)
    }
    ...
    func open() {
        self.notchSize = CGSize(width: openNotchSize.width, height: CGFloat(Defaults[.customOpenHeight]))
