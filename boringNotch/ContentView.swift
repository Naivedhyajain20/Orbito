...
struct ContentView: View {
    ...
    private var computedChinWidth: CGFloat {
        ...
        {
            chinWidth += (max(0, vm.effectiveClosedNotchHeight - 12) + 68)
        } else if !coordinator.expandingView.show && vm.notchState == .closed
        ...
        ZStack(alignment: .top) {
            VStack(spacing: 0) {
                let mainLayout = NotchLayout()
                    ...
                    .padding(
                        .horizontal,
                        vm.notchState == .open ? 8 : cornerRadiusInsets.closed.bottom
                    )
                    .padding(.bottom, vm.notchState == .open ? 2 : 0)
            ...
            VStack(alignment: .leading) {
                if coordinator.helloAnimationRunning {
                        ...
                        && vm.notchState == .closed && Defaults[.showPowerStatusNotifications]
                    {
                      ...
                      } else if !coordinator.expandingView.show && vm.notchState == .closed && timerManager.hasActiveLiveActivity && !vm.hideOnClosed {
                          TimerLiveActivity()
                              .frame(alignment: .center)
              ...
              .zIndex(2)
            if vm.notchState == .open {
                NotchNestRootView(albumArtNamespace: albumArtNamespace)
                    .transition(
    ...
    func TimerLiveActivity() -> some View {
        HStack(spacing: 0) {
            // Left ear: Icon with glowing circular tint
            HStack {
                ZStack {
                    Circle()
...
struct GeneralDropTargetDelegate: DropDelegate {
    ...
    func performDrop(info: DropInfo) -> Bool {
        isTargeted = false
        ShelfStateViewModel.shared.load(info.itemProviders(for: [.fileURL, .url, .utf8PlainText, .plainText, .data]))
        return true
    }
}
