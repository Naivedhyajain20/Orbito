...
class MusicManager: ObservableObject {
    ...
    private func startPlaybackTimer() {
        ...
        playbackTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: true) { [weak self] _ in
            Task { @MainActor [weak self] in
                guard let self = self, self.isPlaying, self.songDuration > 0 else { return }
                let newTime = min(self.elapsedTime + 0.5 * Double(self.playbackRate), self.songDuration)
    ...
    private func updateFromPlaybackState(_ state: PlaybackState) {
        ...
        } else if state.isPlaying && playbackTimer == nil {
            self.startPlaybackTimer()
        }
        ...
        if hasContentChange {
            ...
            } else if state.artwork == nil && (titleChanged || artistChanged) {
                // Try to use app icon if no artwork but track actually changed
            ...
            if artworkChanged || (state.artwork == nil && (titleChanged || artistChanged)) {
